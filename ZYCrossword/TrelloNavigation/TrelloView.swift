...
public class TrelloView: UIView, UIScrollViewDelegate {
    ...
    public init(frame: CGRect, tabCount: Int,trelloTabCells: TrelloTabCells) {
        ...
        self.addSubviews(views: tabView, listView)
        ...
        for i in 0 ..< tabCount {
            ...
            tableView.headerDidFolded = { folded in
                if self.isFoldedMode != folded {
                    ...
                    TrelloAnimate.tabUnfold(trelloView: self, unfold: !folded, finish: { (unfolded) in
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView == listView {
            ...
            if x.truncatingRemainder(dividingBy: width)  == 0 {
    ...
    func prepareAnimate() {
        tabView.didClickIndex = { index in
            ...
            TrelloAnimate.tabUnfold(trelloView: self, unfold: self.isFoldedMode, finish: { (unfolded) in
                self.isFoldedMode = unfolded
            })
