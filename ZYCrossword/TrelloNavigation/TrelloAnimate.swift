    ...
    static func tabUnfold(trelloView: TrelloView, unfold: Bool, finish: @escaping Unfolded) {
        UIView.animate(withDuration: 0.3, animations: {
            ...
            _ = trelloView.tabView.subviews.map { view in
                ...
                (view.frame, view.titleLabel.frame, view.boardView.frame) = unfold ? (CGRect(x: nextX, y: 0, width: ScreenWidth / 4.5, height: CGFloat(view.heightLevel) * 20.0 + 50.0), CGRect(x: ScreenWidth / 27.0, y: 10.0, width: ScreenWidth / (4.5 * 1.5), height: 25.0), CGRect(x: ScreenWidth / 18.0, y: view.titleLabel.bottom + 10.0, width: ScreenWidth / 9.0, height: CGFloat(view.heightLevel) * 20.0)) : (CGRect(x: nextX, y: 0, width: 30.0, height: 30.0 + CGFloat(view.heightLevel) * 10.0 + 20.0 ), CGRect(x: 5.0, y: 0, width: 20.0, height: 10.0), CGRect(x: 5.0, y: 5.0, width: 20.0, height: CGFloat(view.heightLevel) * 10.0))
                nextX += view.width
